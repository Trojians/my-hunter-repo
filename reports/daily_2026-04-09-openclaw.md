# 🚀 GitHub AI 猎手简报 (2026-04-09)

本次捕获共计 **9** 个热门项目。

---

### [models](https://github.com/arimxyer/models) ⭐375
> TUI and CLI for browsing AI models, benchmarks, coding agents, and statuses for AI providers.

### [OfficeCLI](https://github.com/iOfficeAI/OfficeCLI) ⭐1537
> OfficeCLI is the first and best command-line tool purpose-built for AI agents to read, edit, and automate Word, Excel, and PowerPoint files. Free, open-source, single binary, no Office installation required.

### [openclaw](https://github.com/openclaw/openclaw) ⭐352248
> Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 

### [claude-usage](https://github.com/phuryn/claude-usage) ⭐595
> A local dashboard for tracking your Claude Code token usage, costs, and session history. Pro and Max subscribers get a progress bar. This gives you the full picture.

### [token-optimizer](https://github.com/alexgreensh/token-optimizer) ⭐227
> Find the ghost tokens. Fix them. Survive compaction. Avoid context quality decay. 

### [cursor-free-vip](https://github.com/SHANMUGAM070106/cursor-free-vip) ⭐1549
> [Support 0.48.x]（Reset Cursor AI MachineID & Auto Sign Up / In & Bypass Higher Token Limit）自动注册 Cursor Ai ，自动重置机器ID ， 免费升级使用Pro功能: You've reached your trial request limit. / Too many free trial accounts used on this machine. Please upgrade to pro. We have this limit in place to prevent abuse. Please let us know if you believe this is a mistake.

### [abshare.github.io](https://github.com/abshare/abshare.github.io) ⭐937
> 每日分享免费节点、免费机场、ssr节点、v2ray节点、v2ray订阅、clash节点、clash订阅、shadowrocket订阅、Quantumult X订阅、Clash .NET订阅、小火箭节点、小猫咪节点、免费翻墙、免费科学上网、免费梯子、免费trojan节点、蓝灯、谷歌商店、翻墙梯子、安卓VPN、iphone翻墙节点、iphone vpn、一键翻墙浏览器、节点分享、免费SSR、蓝灯、谷歌商店、V2ary免费节点、代理、proxy代理科学上网、TG代理、电报代理、Telegram代理、ip加速、翻墙软件、socks5、破解VPN、机场推荐、节点订阅、破解VPN、ChatGPT 

### [cn-news](https://github.com/hello-world-1989/cn-news) ⭐362
> V2Ray Clash订阅链接, Outline, 大翻墙运动, 拆除防火墙,拆墙运动,李老师不是你老师 推文 每小时更新 , 完全免费vpn, 免费科学上网, 翻墙,科学上网,免费翻墙, 免费VPN,开源vpn, free vpn,好用VPN

### [xray-proxy-grabber-telegram](https://github.com/MrMohebi/xray-proxy-grabber-telegram) ⭐279
> Xray proxy grabber and checker + JSON and YAML Config - telegram channels

